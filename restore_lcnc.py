            ...
            if cls:
                ...
                for n in dest.findall("glade-widget-class"):
                    if n.get("name") == "NCam":
                        classfounded = True
                        print(
                            "glade-widget-class named NCam already exists in %s\n" % s
                        )
                        break
                if not classfounded:
            ...
            if cls:
                ...
                for n in dest.findall("glade-widget-class-ref"):
                    if n.get("name") == "NCam":
                        classfounded = True
                        print(
                            "glade-widget-class-ref named NCam already exists in %s" % s
                        )
                        break
                if not classfounded:
                    dest.insert(
                        0, etree.fromstring('<glade-widget-class-ref name="NCam"/>')
                    )
                    edited = True
