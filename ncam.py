...
from gi.repository import Gtk, Gio, Gdk, Pango, GdkPixbuf, GObject, GLib  # noqa: E402
...
# info at http://www.pyGtk.org/pyGtk.reference/Pango.markup-language.html
...
def mess_yesno(mess, title=""):
    return (
        mess_with_buttons(
            mess,
            (_("_Yes"), Gtk.ResponseType.YES, _("_No"), Gtk.ResponseType.NO),
            title,
        )
...
def copy_dir_recursive(
    ...
    for p in os.listdir(fromdir):
        ...
        if os.path.isdir(frompath):
            mode, update_ct = copy_dir_recursive(
                frompath,
                topath,
                update_ct=update_ct,
                mode=mode,
        ...
        if not os.path.isfile(topath) or overwrite:
            ...
            if (
                hashlib.md5(open(frompath, "rb").read()).digest()
                ...
                if os.path.getctime(frompath) < os.path.getctime(topath):
                    ...
                    if verbose:
                        print(
                            _("Keeping modified local file %(filename)s")
                            % {"filename": p}
                        )
                    pass
...
def require_ini_items(fname, ini_instance):
    ...
    if val is None:
        msg = _(
            "There is no PROGRAM_PREFIX in ini file\n"
...
def create_M_file():
    ...
    with open(p, "w") as f:
        ...
        f.write("dlg.add_button(_('_Ok'), Gtk.ResponseType.OK).grab_focus()\n\n")
...
class VKB(object):
    def __init__(self, toplevel, tooltip, min_value, max_value, data_type, convertible):
        ...
        self.dlg.set_decorated(True) # Title bar
        # self.dlg.set_transient_for(win) # win è la finestra che ha aperto il dialog -- VEDERE
        self.dlg.set_border_width(3)
        self.dlg.set_default_size(250, 250)
        #self.dlg.set_property("skip-taskbar-hint", True)
        self.dlg.set_keep_above(True) # In primo piano. Qui non funziona. Spostato dopo show_all()

        lbl = Gtk.Label(label="Etichetta lbl")
        lbl.set_line_wrap(True)
        self.dlg.vbox.pack_start(lbl, False, False, 0)
        lbl.set_markup(tooltip)

        self.entry = Gtk.Label(label="Label Entry") # Display della calcolatrice
        #self.entry = Gtk.Entry() # Display della calcolatrice
        ...
        box = Gtk.EventBox()
        box.override_background_color(Gtk.StateType.NORMAL, Gdk.RGBA(1.0, 1.0, 1.0, 1.0))
        ...
        self.dlg.vbox.pack_start(tbl, False, False, 0) # DEBUG vedere
        ...
        #self.dlg.set_keep_above(True) # DEBUG vedere se serve
    ...
    def run(self, not_allowed):
        ...
        self.dlg.show_all() # Visualizza la calcolatrice
        self.dlg.set_keep_above(True)
        
        self.opened_paren = 0

        if True: # DEBUG per non spostare l'indentazione
                    ...
                    if not_allowed is not None:
                        for na in not_allowed.split(":"):
                            if get_float(na) == rval:
                                ...
                                break
                    if is_good:
                        # print("debug vkb run OK return: ", type(response), response, type(rval), rval)
                        return response, str(rval)
                        #return response, rval
            else:
                print("debug vkb run 2", response)
    ...
    def key_press_event(self, win, event): # Tasti fisici tastiera
...
class CellRendererMx(Gtk.CellRendererText):
    ...
    def edit_number(self, time_out=0.05):
        ...
        with VKB(
            ...
            if not initialize:
                if ((self.data_type == "int") and (decimal_point in self.inputKey)) or (
                    self.inputKey == "BS"
                ):
            ...
            time.sleep(time_out) # Attesa prima di aprire la calcolatrice
            not_allowed = None
            print("debug 2 vkb")

            # Chiama la calcolatrice
            response, valore_inserito = vkb.run(not_allowed)
            print("Debug edit_number risultato da vkb.run()", type(response), response, type(valore_inserito), valore_inserito)
            ...
            return response, str(valore_inserito)
    ...
    def edit_string(self, time_out=0.05):
        ...
        self.dialogBox = self.stringedit_window.get_content_area()

        self.stringedit_entry = Gtk.Entry() # Text entry
        self.stringedit_entry.set_editable(True)
        self.dialogBox.pack_end(self.stringedit_entry, False, False, 0)

        # position the popup entry text on the edited cell
        _, tree_x, tree_y = self.tv.get_bin_window().get_origin()
        tree_w, tree_h = self.tv.get_bin_window().get_geometry()[2:4]
        ...
        self.stringedit_window.set_keep_above(True)
    ...
    def do_start_editing(self, event, treeview, path, background_area, cell_area, flags):
        ...
        if self.editdata_type in NUMBER_TYPES:
            print("Debug do_start_editing 1 ")
            response, result = self.edit_number()

            print("Debug response, result", response, result) # DEBUG
            if response == Gtk.ResponseType.OK:
                print("response == Gtk.ResponseType.OK") # DEBUG
                self.edited(self, path, result)
                
            self.do_start_editing() # DEBUG Anche se genera errore, sembra risolvere il crash (perché?)
            return None
            ...
            print("do_start_editing string, gcode") #DEBUG
...
# GObject.type_register(CellRendererMx) # DEBUG Serve?
...
class Parameter(object):
    ...
    def __repr__(self):
        # Vedere se serve .decode('utf-8')
        # return etree.tostring(self.to_xml(), pretty_print=True)
        return etree.tostring(self.to_xml(), pretty_print=True).decode('utf-8')
    ...
    def get_display_string(self):
        if self.get_type() == "float":
            if default_metric and "metric_value" in self.attr:
                return get_string(
                    get_float(self.attr["value"]) * 25.4, self.get_digits()
                )
            else:
    ...
    def get_min_value(self):
        min_v = (
            self.attr["minimum_value"] if "minimum_value" in self.attr else "-999999.9"
        )
        if (
            self.get_type() == "float"
            and default_metric
            and "metric_value" in self.attr
        ):
    ...
    def get_max_value(self):
        max_v = (
            self.attr["maximum_value"] if "maximum_value" in self.attr else "999999.9"
        )
        if (
            self.get_type() == "float"
            and default_metric
...
class Feature(object):
    ...
    def __repr__(self):
        # Vedere quale riga è giusta come return
        # return etree.tostring(self.to_xml(), pretty_print=True)
        return etree.tostring(self.to_xml(), pretty_print=True).decode('utf-8')
        # return str(etree)
    ...
    def get_tooltip(self):
        s = (
            _(self.attr["tool_tip"])
            if "tool_tip" in self.attr
            else _(self.attr["help"])
            if "help" in self.attr
            else None
        )
        return s.replace("&#176;", "°")
    ...
    def from_src(self, src):
        ...
        self.attr["order"] = [
            s if s[:6] == "PARAM_" else "PARAM_" + s for s in self.attr["order"]
        ]
        ...
        parameters = self.attr["order"] + [
            p for p in conf if (p[:6] == "PARAM_" and p not in self.attr["order"])
        ]
        for s in parameters:
            if s in conf:
                pn = s.lower()
        ...
        for l in ["DEFINITIONS", "BEFORE", "CALL", "AFTER", "VALIDATION", "INIT"]:  # noqa: E741
            if l in conf and "content" in conf[l]:
                self.attr[l.lower()] = re.sub(
                    r"(?m)\r?\n\r?\.", "\n", conf[l]["content"]
                )
            else:
                self.attr[l.lower()] = ""
    ...
    def get_id(self, xml):
        ...
        if xml is not None:
            ...
            l = xml.findall(".//feature[@type='%s']" % self.attr["type"])  # noqa: E741
    ...
    def msg_inv(self, msg, msgid):
        msg = msg.replace("&#176;", "°")
        print(
            "\n%(feature_name)s : %(msg)s"
            % {"feature_name": self.get_name(), "msg": msg}
        )

        if (
            ("ALL:msgid-0" in EXCL_MESSAGES)
            or ("%s:msgid-0" % (self.get_type()) in EXCL_MESSAGES)
            or (("%s:msgid-%d" % (self.get_type(), msgid)) in EXCL_MESSAGES)
        ):
            return
        ...
        dlg.get_content_area().pack_start(cb, True, False, 0)
    ...
    def check_hash(self, s, default=0):
        try:
            ...
            print(
                _("%(feature_name)s : can not evaluate %(value)s")
...
class Preferences(object):
    ...
    def read(self, cat_name, read_all=True):
        ...
        if read_all:
            ...
            self.pref_file = os.path.join(
                NCAM_DIR, CATALOGS_DIR, self.cat_name, PREFERENCES_FILE
            )

            config.read(self.cfg_file)

            self.w_adj_value = read_int(config, "display", "width", 550)
            self.col_width_adj_value = read_int(
                config, "display", "name_col_width", 160
            )
            self.tv_w_adj_value = read_int(config, "display", "master_tv_width", 175)
            self.restore_expand_state = read_boolean(
                config, "display", "restore_expand_state", True
            )
            self.tv2_expandable = read_boolean(
                config, "display", "tv2_expandable", False
            )
            ...
            self.sub_hdrs_in_tv1 = read_boolean(
                config, "layout", "subheaders_in_master", False
            )
            self.hide_value_column = read_boolean(
                config, "layout", "hide_value_column", False
            )
            ...
            vkb_cancel_on_out = read_boolean(
                config, "virtual_kb", "cancel_on_focus_out", True
            )
    ...
    def add_excluded_msg(self, ftype, msgid):
        ...
        with open(fname, "wb") as configfile:
            parser.write(configfile.encode("utf-8"))
    ...
    def create_defaults(self):
        ...
        if self.cat_name == "mill":
            ...
            self.default += (
                "#<_probe_func>              = 38." + self.ngc_probe_func + "\n"
            )
            self.default += (
                "#<_probe_feed>              = " + self.ngc_probe_feed + "\n"
            )
            self.default += (
                "#<_probe_latch>             = " + self.ngc_probe_latch + "\n"
            )
            self.default += (
                "#<_probe_latch_feed>        = " + self.ngc_probe_latch_feed + "\n"
            )
            self.default += (
                "#<_probe_safe>              = " + self.ngc_probe_safe + "\n"
            )
            self.default += (
                "#<_probe_tip_dia>           = " + self.ngc_probe_tip_dia + "\n\n"
            )
            self.default += (
                "#<_probe_tool_len_comp>     = " + self.probe_tool_len_comp + "\n"
            )
            self.default += (
                "#<probe_height>             = " + self.ngc_probe_height + "\n"
            )
        ...
        if self.cat_name in ["mill", "plasma"]:
            ...
            self.default += (
                "#<_show_final_cuts>         = " + self.ngc_show_final_cut + "\n"
            )
            self.default += (
                "#<_show_bottom_cut>         = " + self.ngc_show_bottom_cut + "\n\n"
            )

            self.default += (
                "#<_spindle_all_time>        = " + self.spindle_all_time + "\n\n"
            )

        if self.cat_name in ["mill", "lathe"]:
            self.default += (
                "#<_spindle_speed_up_delay>  = "
                + self.ngc_spindle_speedup_time
                + "\n\n"
            )

        if self.cat_name == "plasma":
            self.default += (
                "#<_plasma_test_mode>        = " + self.plasma_test_mode + "\n\n"
            )
...
class NCam(Gtk.VBox):
    ...
    def __init__(self, *a, **kw):
        ...
        if no_ini:
            ...
            try:
                ...
                err_exit(
                    _("Open fails for ini file : %(inifilename)s\n\n%(detail)s")
            ...
            self.pref.ngc_init_str = ini_instance.find(
            ...
            machine_metric = ini_instance.find("TRAJ", "LINEAR_UNITS") in [
                "mm",
                "metric",
            ]
        ...
        if cat_dir_name is not None:
            ...
            print(
                _("Using default %(mnu)s,  no %(dir)s/menu-custom.xml found\n")
    ...
    def update_user_tree(self, fromdirs, todir):
        ...
        for d in fromdirs:
            ...
            mode, update_ct = copy_dir_recursive(
                os.path.join(SYS_DIR, d),
                os.path.join(todir, d),
                update_ct=0,
                mode=mode,
                overwrite=False,
                verbose=False,
        ...
        for s in VALID_CATALOGS:
            ...
            if os.path.exists(srcdir):
                dst = os.path.join(
                    NCAM_DIR, CATALOGS_DIR, s, PROJECTS_DIR, EXAMPLES_DIR
                )
    ...
    def validation_menu_activate(self, *arg):
        ...
        if self.selected_feature is not None:
            self.actionValFeatDlg.set_label(
                "%s %s" % (_("Show All For"), self.selected_feature.get_name())
            )
            self.actionValFeatNone.set_label(
                "%s %s" % (_("Show None For"), self.selected_feature.get_name())
            )

    def view_menu_activate(self, *arg):
        self.agrp_mi.set_visible(
            self.selected_type in ["sub-header", "header"]
            and self.actionDualView.get_active()
        )
    ...
    def action_lcncForum(self, *arg):
        webbrowser.open(
            "http://www.linuxcnc.org/index.php/english/forum/40-subroutines-and-ngcgui"
        )
    ...
    def action_preferences(self, *arg):
        ...
        old_view = self.actionDualView.get_active()  # noqa: F841
        ...
        if self.pref.edit(self):
            ...
            if old_treeview_icon_size != treeview_icon_size:
                self.tv1_icon_cell.set_fixed_size(
                    treeview_icon_size, treeview_icon_size
                )
                if self.treeview2 is not None:
                    self.tv2_icon_cell.set_fixed_size(
                        treeview_icon_size, treeview_icon_size
                    )
            if (old_menu_icon_size != menu_icon_size) or (
                old_add_menu_icon_size != add_menu_icon_size
            ):
                self.create_menubar()
    ...
    def update_catalog(self, xml=None):
        ...
        if self.catalog_path != self.catalog_src:
            self.icon_store.append(
                [
                    get_pixbuf("upper-level.png", add_dlg_icon_size),
                    "parent",
                    _("Back..."),
                    "parent",
                    0,
        ...
        for path in range(len(self.catalog_path)):
            ...
            if p.tag.lower() in ["menuitem", "menu", "group", "sub"]:
                ...
                self.icon_store.append(
                    [
                        get_pixbuf(p.get("icon"), add_dlg_icon_size),
                        p.tag.lower(),
                        _(name),
                        src,
                        path,
            ...
            for ptr in range(len(path)):
                try:
                    ...
                    if p.tag.lower() in ["menu", "menuitem", "group", "sub"]:
                        ...
                        tooltip = (
                            _(p.get("tool_tip")) if "tool_tip" in p.keys() else None
                        )
    ...
    def create_treeview(self):
        ...
        col.pack_start(self.name_cell, True)
    ...
    def action_saveUser(self, *arg):
        fname = os.path.join(
            NCAM_DIR, CATALOGS_DIR, self.catalog_dir, USER_DEFAULT_FILE
        )
        parser = configparser.ConfigParser()
        parser.read(fname)
    ...
    def action_deleteUser(self, *arg):
        ...
        if parser.has_section(section):
            ...
            with open(fname, "w") as configfile:
                parser.write(configfile)
            self.pref.read_user_values()
            self.actionDeleteUser.set_sensitive(
                self.selected_feature.get_type() in USER_SUBROUTINES
            )

    def action_saveCurrent(self, *arg):
        fname = os.path.join(
            NCAM_DIR, CATALOGS_DIR, self.catalog_dir, PROJECTS_DIR, CURRENT_WORK
        )
    ...
    def pop_menu(self, tv, event):
        if event.button == 3:
            ...
            # Tasto destro del mouse su una riga del treeview
            if path is not None:
                path = path[0]
                tv.set_cursor(path)
            else:
    ...
    def create_second_treeview(self):
        ...
        col.pack_start(self.name_cell2, True)
...
#               act.set_icon_name(label)
        ...
        # "<control>X"
        self.actionProject = ca(
            "Project", None, _("_Projects"), None, None, None)
        self.actionNew = ca(
            "New", None, _("_New"), "<control>N", None, self.action_new_project)
        self.actionOpen = ca(
            "Open", None, _("_Open..."), "<control>O", None, self.action_open_project, 0,)
        self.actionOpenExample = ca(
            "OpenExample", None, _("Open Example..."), "", _("Open Example Project"), self.action_open_project, 1, )
        self.actionSave = ca(
            "Save", None, _("_Save as..."), "<control>S", _("Save project as xml file"), self.action_save_project, )
        self.actionSaveTemplate = ca(
            "SaveTemplate", None, _("Save as Default Template"), "", _("Save project as default template"), self.action_save_template, )
        self.actionSaveNGC = ca(
            "SaveNGC", None, _("Export gcode as RS274NGC..."), "", _("Export gcode as RS274NGC"), self.action_save_ngc, )

        # actions related to editing
        self.actionEditMenu = ca(
            "EditMenu", None, _("_Edit"), None, None, self.edit_menu_activate
            )
        self.actionUndo = ca(
            "Undo", _("edit-undo"), _("_Undo"), "<control>Z", _("Undo last operation"), self.action_undo,
            )
        self.actionRedo = ca(
            "Redo", _("edit-redo"), _("_Redo"), "<control><shift>Z", _("Cancel last Undo"), self.action_redo,
            )
        self.actionCut = ca(
            "Cut", _("edit-cut"), _("_Cut"), "<control>X", _("Cut selected subroutine to clipboard"), self.action_cut,
            )
        self.actionCopy = ca(
            "Copy", _("edit-copy"), _("_Copy"), "<control>C", _("Copy selected subroutine to clipboard"), self.action_copy,
            )
        self.actionPaste = ca(
            "Paste", _("edit-paste"), _("_Paste"),  "<control>V", _("Paste from clipboard"), self.action_paste,
            )
        self.actionAdd = ca(
            "Add", _("list-add"), _("_Add"), "<control>Insert", _("Add a subroutine"), self.action_add,
            )
        self.actionDuplicate = ca(
            "Duplicate", _("edit-copy"), _("_Duplicate"), "<control>D", _("Duplicate selected subroutine"), self.action_duplicate,
            )
        self.actionDelete = ca(
            "Delete", _("list-remove"), _("_Remove"), "<control>Delete", _("Remove selected subroutine"), self.action_delete,
            )
        self.actionAppendItm = ca(
            "AppendItm", _("format-indent-more"), _("_Indent"), "<control>Right", _("Add to Items"), self.action_appendItm, )
        self.actionRemoveItm = ca(
            "RemoveItm", _("format-indent-less"), _("_Unindent"), "<control>Left", _("Remove from Items"), self.action_removeItem,)
        self.actionMoveUp = ca(
            "MoveUp", _("go-up"), _("_Go up"), "<control>Up", _("Move up"), self.move, 1,)
        self.actionMoveDown = ca(
            "MoveDown", _("go-down"), _("_Go down"), "<control>Down", _("Move down"), self.move, -1,)
        self.actionSaveUser = ca(
            "SaveUser", _("_Save"), _("Save Values of this Subroutine as Defaults"), "", _("document-save"), self.action_saveUser,)
        self.actionDeleteUser = ca(
            "DeleteUser", _("_Cancel"), _("Delete Custom Default Values"), None,
            _("Delete Custom Default Values"), self.action_deleteUser)
        ...
        self.actionLoadCfg = ca(
            "LoadCfg", _("_Open"), _("Add a Prototype Subroutine..."), "",
                _("Add a Subroutine Definition File"), self.action_loadCfg,
            )
        self.actionImportXML = ca(
            "ImportXML", _("_Revert_to_saved"), _("Import a Project File..."), None,
                _("Import a Project Into the Current One"), self.action_importXML,
            )

        # actions related to view
        self.actionViewMenu = ca(
            "ViewMenu", None, _("_View"), None, None, self.view_menu_activate
        )
        self.actionCollapse = ca(
            "Collapse",  _("_Zoom_out"), _("Collapse All Other Nodes"),
            "<control>K", _("Collapse All Other Nodes"), self.action_collapse,)
        self.actionSaveLayout = ca(
            "SaveLayout",
            _("_Save"),
            _("Save As Default Layout"),
            "",
            _("Save As Default Layout"),
            self.action_saveLayout,
        )

        self.action_group.add_radio_actions(
            [
                ("SingleView", None, _("Single View"), None, None, 1),
                ("DualView", None, _("Dual View"), None, None, 2),
            ],
            1,
            self.set_layout,
        )
        ...
        self.actionUtilMenu = ca(
            "UtilitiesMenu", None, _("_Utilities"), None, None, self.utilMenu_activate
        )
        self.actionLoadTools = ca(
            "LoadTools",
            _("_Refresh"),
            _("Reload Tool Table"),
            None,
        ...
        self.actionPreferences = ca(
            "Preferences", _("_Preferences"), _("Edit Preferences..."), None, _("Edit Preferences"), self.action_preferences,
            )
        ...
        self.actionChUnits = ca(
            "ChUnits", None, _("Change Units"), None, _(""), self.action_chUnits
        )

        # actions related to validations
        self.actionValidationMenu = ca(
            "ValidationMenu",
            _("_Info"),
            _("_Validation Messages"),
            None,
            None,
            self.validation_menu_activate,
        )
        self.actionValAllDlg = ca(
            "ValAllDlg",
            _("_Yes"),
            _("Show All"),
            None,
            _("Show All Non-validation Messages"),
            self.action_ValAllDlg,
        )
        self.actionValNoDlg = ca(
            "ValNoDlg",
            _("_No"),
            _("Show None"),
            None,
            _("Do Not Show Any Messages"),
            self.action_ValNoDlg,
        )
        self.actionValFeatDlg = ca(
            "ValFeatDlg",
            _("_Yes"),
            _("Show All For Current Type"),
            None,
            None,
            self.action_ValFeatDlg,
        )
        self.actionValFeatNone = ca(
            "ValFeatNone",
            _("_No"),
            _("Show None For Current Type"),
            None,
            None,
            self.action_ValFeatNone,
        )

        # actions related to help
        self.actionHelpMenu = ca(
            "HelpMenu", None, _("_Help"), None, None, None)
        self.actionYouTube = ca(
            "YouTube", None, _("NativeCAM on YouTube"), None, None, self.action_youTube)
        self.actionYouTrans = ca(
            "YouTranslate", None, _("Translating NativeCAM"), None, None, self.action_youTrans, )
        self.actionCNCHome = ca(
            "CNCHome", None, _("LinuxCNC web Site"), None, None, self.action_lcncHome )
        self.actionForum = ca(
            "CNCForum", None, _("LinuxCNC Forum"), None, None, self.action_lcncForum )
        self.actionAbout = ca(
            "About", None, _("_About"), None, None, self.action_about)

        # actions related to toolbars and popup
        self.actionHideField = ca(
            "HideField", None, _("Hide Selected Field"), None, _("Hide Selected Field"), self.action_hideField,
            )
        self.actionShowF = ca(
            "ShowFields",
            None,
            ...
            None,
            _("Show All Fields"),
            self.action_showFields,
        )
        self.actionCurrent = ca(
            "Current",
            _("_Save"),
            _("Save Project as Current Work"),
            "",
            _("Save Project as Current Work"),
            self.action_saveCurrent,
        )
        self.actionBuild = ca(
            "Build", _("document-save"), _("_Execute"), None,
            _("Generate %(filename)s and load it in LinuxCNC") % {"filename": GENERATED_FILE}, self.action_build,
            )
        self.actionRename = ca(
            "Rename",
            None,
            _("Rename Subroutine"),
            None,
        ...
        self.actionChngGrp = ca(
            "ChngGrp",
            None,
            _("Group <-- --> Sub-group"),
            None,
            _("Group <-- --> Sub-group"),
            self.action_chng_group,
        )
        self.actionDataType = ca(
            "DataType",
            None,
            _("Change to GCode"),
            None,
        ...
        self.actionRevertType = ca(
            "RevertType",
            None,
            _("Revert to original type"),
            None,
            _("Revert to original type"),
            self.action_revert_type,
        )
    ...
    def get_selected_feature(self, widget):
        ...
        if itr is not None:
            ...
            self.selected_feature_ts_path_s = self.treestore.get_string_from_iter(
                ts_itr
            )
            ...
            if self.selected_feature_parent_itr:
                ...
                self.can_remove_from_group = (
                    self.iter_selected_type == tv_select.feature
                ) and model.get_value(self.selected_feature_parent_itr, 0).attr.get(
                    "type"
                ) == "items"
            else:
                path_parent = None
            ...
            if index_s:
                if path_parent is None:
                    ...
                    path_previous = path_parent[0 : depth - 1] + list((index_s - 1,))
        ...
        if self.actionDualView.get_active():
            ...
            if (
                ...
                and (
                    ...
                    in [tv_select.items, tv_select.feature, tv_select.header]
                )
    ...
    def action_removeItem(self, *arg):
        ...
        while (
            parent != xml
            and not (parent.tag == "param" and parent.get("type") == "items")
    ...
    def expand_and_select(self, path):
        treepath = Gtk.TreePath.new_from_indices((0,))
        if path is not None:
            treepath = Gtk.TreePath.new_from_indices(path)
    ...
    def col_width_adj_value_changed(self, *arg):
        ...
        if self.treeview2 is not None:
            self.treeview2.get_column(0).set_min_width(
                int(self.col_width_adj.get_value())
            )
            ...
            if keyname == "Tab" and self.treeview2 is not None:
                if widget == self.treeview:
                    self.hint_label.set_markup(
                        items_fmt_str % _("Secondary treeview focused")
                    )
                    ...
                    self.hint_label.set_markup(
                        items_fmt_str % _("Primary treeview focused")
                    )
                    self.treeview.grab_focus()
                    model, itr = self.treeview.get_selection().get_selected()
                    p = model.get_path(itr)
                ...
                if path != (0,): # DEBUG vedere errore tuple o path
                    depth = len(path)
                    ...
                    if index_s > 0:
                        ...
                        while iter_p is not None:
                            ...
                            if (count == 0) or not widget.row_expanded(
                                model.get_path(iter_p)
                            ):
                ...
                widget.set_cursor_on_cell(
                    path, focus_column=widget.get_column(1), focus_cell=None, start_editing=True
                )
                #widget.set_cursor_on_cell(
                #    widget, path, focus_column=widget.get_column(1), start_editing=True
                #)

            elif keyname == "BackSpace":
                widget.get_column(1).get_cell_renderers()[0].set_Input("BS")
                widget.set_cursor_on_cell(
                    #path, focus_column=widget.get_column(1), start_editing=True
                    path, focus_column=widget.get_column(1), focus_cell=None, start_editing=True
                )
    ...
    def treestore_from_xml(self, xml):
        treestore = Gtk.TreeStore(
            object, str, GObject.TYPE_BOOLEAN, GObject.TYPE_BOOLEAN
        )
        ...
        def recursive(itr, xmlpath):
            for xml in xmlpath:
                if xml.tag == "feature":
                    ...
                    for p in f.param:
                        header_name = (
                            p.attr["header"].lower() if "header" in p.attr else ""
                        )

                        tool_tip = p.get_tooltip() if "tool_tip" in p.attr else None
                        p_type = p.get_type()
                        p_hidden = get_int(
                            p.attr["hidden"] if "hidden" in p.attr else "0"
                        )

                        if self.actionDualView.get_active():
                            if self.actionSubHdrs.get_active():
                                m_visible = (
                                    p_type in ["items", "header", "sub-header"]
                                    and not p_hidden
                                )
                                is_visible = (
                                    p_type not in ["items", "header", "sub-header"]
                                    and not p_hidden
                                )
                            else:
                                m_visible = (
                                    p_type in ["items", "header"] and not p_hidden
                                )
                                is_visible = (
                                    p_type not in ["items", "header"] and not p_hidden
                                )
                        else:
    ...
    def edited(self, renderer, path, new_value):
        ...
        # value_changed = False # DEBUG Togliere?
        ...
        if value_set:
            self.refresh_views()
            self.action()
        self.focused_widget.grab_focus() #DEBUG crash
        # print("Grab focus") # DEBUG
    ...
    def action_renameF(self, *arg):
        self.newnamedlg = Gtk.MessageDialog(
            parent=None,
            modal=True,
        ...
        self.newnamedlg.set_markup("Enter new name for")
    ...
    def import_xml(self, xml_i):
        ...
        if xml_i is not None:
            ...
            if self.iter_selected_type == tv_select.none:
                ...
                dest = xml.find(
                    ".//*[@path='%s']/param[@type='items']" % self.items_ts_parent_s
                )
                opt = 2
                i = -1
                ...
                if l_path == 1:
                    next_path = Gtk.TreePath.new_from_indices((i + 1,))
                else:
                    new_indeces = self.selected_feature_path[0 : l_path - 1] + (
                        self.selected_feature_path[l_path - 1] + 1,
                    )
                    next_path = Gtk.TreePath.new_from_indices(new_indeces)
    ...
    def add_feature(self, widget, src):
        ...
        if (".//%s" % XML_TAG) in src_data:
            ...
            mess_dlg(
                _("'%(source_file)s' is not a valid cfg or xml file")
    ...
    def autorefresh_call(self, *arg):
        ...
        try:
            ...
            if stat.interp_state == linuxcnc.INTERP_IDLE:
                try:
                    Tkinter.Tk().tk.call(
                        "send", "axis", ("remote", "open_file_name", fname)
                    )
                    ...
                    if stat.task_mode == linuxcnc.MODE_AUTO:
                        ...
                        mess_dlg(
                            _(
                                "LinuxCNC could not change to AUTO mode. Generated NC file was not loaded."
                            )
                        )
        except Exception:
            self.actionAutoRefresh.set_active(False)
    ...
    def action(self, xml=None, refresh=True):
        ...
        self.undo_list.append(etree.tostring(xml).decode('utf-8'))
    ...
    def update_do_btns(self, refresh):
        ...
        if self.actionAutoRefresh.get_active() and refresh:
            self.timeout = GObject.timeout_add(
                self.pref.timeout_value, self.autorefresh_call
            )
    ...
    def action_about(self, *arg):
        ...
        try:
            ver = subprocess.check_output(
                ["dpkg-query", "--show", "--showformat='${Version}'", "nativecam"]
            )
    ...
    def action_save_template(self, *arg):
        ...
        etree.ElementTree(xml).write(
            os.path.join(
                NCAM_DIR, CATALOGS_DIR, self.catalog_dir, PROJECTS_DIR, DEFAULT_TEMPLATE
            ),
            pretty_print=True,
        )
    ...
    def set_layout(self, *arg):
        if self.actionDualView.get_active():
            print("DEBUG actionDualView.get_active")
            if self.treeview2 is None:
                self.create_second_treeview()
        ...
        self.treeview.get_column(1).set_visible(
            self.actionSingleView.get_active() or not self.actionHideCol.get_active()
        )
    ...
    def create_nc_toolbar(self):
        ...
        self.nc_toolbar.set_style(Gtk.ToolbarStyle.BOTH_HORIZ)
        ...
        if model.get_value(itr, 0).get_grayed():
            if data_type == "header":
                ...
                if (
                    ...
                    and not self.actionSubHdrs.get_active()
                ):
    ...
    def get_col_icon(self, column, cell, model, itr, *data):
        if model.get_value(itr, 0).get_type() in NO_ICON_TYPES:
            cell.set_property("pixbuf", None)
        else:
            cell.set_property(
                "pixbuf", model.get_value(itr, 0).get_icon(treeview_icon_size)
            )
    ...
    def treestore_to_xml_recursion(self, itr, xmlpath, allitems=True):
        while itr:
            ...
            while citer:
                ...
                if itm == "items":
                    pa = f.get_attr("path")
                    xmlpath_ = xmlpath.find(
                        ".//*[@path='%s']/param[@type='items']" % pa
                    )
                    if xmlpath_ is not None:
                        self.treestore_to_xml_recursion(
                            self.treestore.iter_children(citer), xmlpath_
                        )
    ...
    def set_expand(self):
        def treestore_set_expand(model, path, itr):
            try:
                ...
                if (
                    "expanded" in p and p["expanded"] == "True"
    ...
    def action_importXML(self, *arg):
        ...
        try:
            ...
            filechooserdialog.set_current_folder(
                os.path.join(NCAM_DIR, CATALOGS_DIR, self.catalog_dir, PROJECTS_DIR)
            )
            filechooserdialog.set_keep_above(True)
            filechooserdialog.set_transient_for(self.get_toplevel())
        ...
        def recursive(xmlpath, dst):
            for xml in xmlpath:
                if xml.tag == "feature":
                    f_A = Feature(xml=xml)
                    src_f = search_path(
                        search_warning.none, f_A.get_attr("src"), CFG_DIR
                    )
                    if src_f is None:
                        f = f_A
                    else:
                                ...
                                for q in f_B.param:
                                    if q.attr["call"] == call:
                                        ...
                                        if "minimum_value" in p.attr:
                                            q.attr["minimum_value"] = p.attr[
                                                "minimum_value"
                                            ]
                                        if "maximum_value" in p.attr:
                                            q.attr["maximum_value"] = p.attr[
                                                "maximum_value"
                                            ]
                                        if "hidden" in p.attr:
    ...
    def action_open_project(self, *arg):
        ...
        if arg[1][0] == 0:  # user project
            ...
            dir_ = os.path.join(
                NCAM_DIR, CATALOGS_DIR, self.catalog_dir, PROJECTS_DIR, EXAMPLES_DIR
            )
    ...
    def set_actions_sensitives(self):
        ...
        self.actionDeleteUser.set_sensitive(
            ...
            and (self.selected_feature.get_type() in USER_SUBROUTINES)
        )
        ...
        self.actionHideField.set_sensitive(
            (self.selected_type in SUPPORTED_DATA_TYPES)
            and (self.selected_type != "items")
        )
        self.actionShowF.set_sensitive(
            self.selected_feature is not None
            and self.selected_feature.has_hidden_fields()
        )
        ...
        try:
            ...
            if dp == "axis":
                ...
                if in_tab:
                    ...
                    try:
                        oldstr = "EMBED_TAB_LOCATION = %s" % parser.get(
                    ...
                    try:
                        oldstr = "EMBED_TAB_NAME = %s" % parser.get(
                            "DISPLAY", "embed_tab_name"
                        )
                        txt = re.sub(r"%s" % oldstr, newstr, txt)
                    except Exception as _:
                        txt = re.sub(r"\[DISPLAY\]", "[DISPLAY]\n" + newstr, txt)
                    ...
                    newstr = (
                        "%sEMBED_TAB_COMMAND = gladevcp -x {XID} -U --catalog=%s %s\n"
                        % (req, ctlog, path2ui)
                    )
                    try:
                        oldstr = "EMBED_TAB_COMMAND = %s" % parser.get(
                            "DISPLAY", "embed_tab_command"
                        )
                        txt = re.sub(r"%s" % oldstr, newstr, txt)
                    except Exception as _:
                        txt = re.sub(r"\[DISPLAY\]", "[DISPLAY]\n" + newstr, txt)
            ...
            if "ncam/my-stuff:ncam/lib/" not in old_sub_path:
                newstr = (
                    "%sSUBROUTINE_PATH = ncam/my-stuff:ncam/lib/%s:ncam/lib/utilities%s\n"
                    % (req, ctlog, old_sub_path)
                )
                try:
                    oldstr = "SUBROUTINE_PATH = " + parser.get(
                        "RS274NGC", "subroutine_path"
                    )
                    txt = re.sub(r"%s" % oldstr, newstr, txt)
                except Exception as _:
